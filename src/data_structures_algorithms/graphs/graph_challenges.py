...
def path_with_max_gold_med(grid: List[List[int]]) -> int:
    """
    ...
    # Used [] for list here as opposed to () for generator which only allows for one pass iteration
    gold_cells = [(r, c) for r in range(ROWS) for c in range(COLS) if grid[r][c] > 0]
    gold_total = sum(grid[r][c] for r, c in gold_cells)
    ...
    def dfs(row: int, col: int) -> int:
        ...
        for r, c in neighbors:
            ...
            if(
                ...
                current_max = max(current_max, dfs(nr, nc))

        grid[row][col] = gold_here
        return gold_here + current_max


    for r, c in gold_cells:
        max_gold = max(max_gold, dfs(r, c))
        if max_gold == gold_total:
            return gold_total
